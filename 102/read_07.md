...
Introduced in later revicions of ES6[], arrows `=>` allow for the use of shorter function syntax.
...
Operators are used for arithmetic and logical computations.
...
### Arithmetic (adding, multiplication, etc.)
...
- `>`/`<`, known as the Comparison operator(s), compares values. Can also be used on strings, which are compared alphabetically.  
...
Ones that have side effects (like assigning values, `x = 10`) and purely evaluative ones (`5 + 10` would resolve by doing the calculation and returning 15, rather than returning `5 = 10`).  
An expression is a unit or block of code that points/leads to a assigned value. Grouped expressions with the use of `()` can override the precedence of operators over others, such as in arithmetic operations.
